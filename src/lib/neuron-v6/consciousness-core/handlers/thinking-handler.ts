...
import { SSMMemoryBridge, createSSMMemoryBridge } from '../../core/ssm-memory-bridge';
import { type DecodedInstruction } from '../../core/ssm-decoder';

// 🆕 导入隐式输出解码器 (Level 3 黑盒)
import {
  ImplicitOutputDecoder,
  type ImplicitOutput,
  type DecodedOutput,
  type DecodePermit,
} from '../../core/implicit-output-decoder';
...
export class ThinkingHandler {
  ...
  // 🆕 Level 3 隐式输出解码器
  private implicitDecoder: ImplicitOutputDecoder;
  ...
  constructor(deps: ThinkingHandlerDeps) {
    ...
    // 🆕 初始化 Level 3 隐式输出解码器
    this.implicitDecoder = new ImplicitOutputDecoder({
      decodeThreshold: 0.618, // 黄金比例
      enableChaos: true, // 启用混沌混淆
      chaosIntensity: 0.05, // 混沌强度
    });
   ...
   */
  async think(input: string, context: ConsciousnessContext): Promise<ThinkingProcess> {
  ...
  private async deepMetaThink(
    ...
    // ─── Step 4: 存储到记忆桥接 ───
    this.memoryBridge.store(thinkingResult.ssmOutput.newState, undefined, 'experience');
    ...
    this.stats.tokenSavingsRate =
      this.stats.totalThinkings > 0 ? this.stats.localDecisions / this.stats.totalThinkings : 0;
  ...
  private async traditionalThink(
    ...
    // 开始元认知监控
    const step1 = this.deps.metacognition.beginThinkingStep('perception', input, '感知输入');
  ...
  async generateResponse(
    ...
    // 检查是否有深度元思考结果
    const metaResult = (thinking as ThinkingProcess & { _metaThinkingResult?: ThinkingResult })
      ._metaThinkingResult;
  ...
  private async generateResponseFromMetaResult(
    ...
    // ─── Step 1: 将 SSM 输出编码为隐式形式 ───
    const implicitOutput = this.implicitDecoder.encode(metaResult.ssmOutput);

    // ─── Step 2: 创建解码许可 ───
    // 根据置信度和外部调用需求决定许可类型
    const permitType = metaResult.needsExternalCall ? 'llm_call' : 'debug';
    const permit = this.implicitDecoder.createPermit(permitType, 1);

    // ─── Step 3: 尝试解码 ───
    const decodedOutput = this.implicitDecoder.decode(implicitOutput, permit);

    if (!decodedOutput) {
      // 解码失败，返回安全响应
      console.warn('[隐式解码] 解码失败，返回默认响应');
      return '我正在思考...';
    }

    // ─── Step 4: 根据解码结果执行 ───
    switch (decodedOutput.type) {
      case 'local_action':
        // 本地决策，不调用LLM
        return this.executeLocalActionFromDecoded(decodedOutput, context);
      ...
      case 'llm_call':
        // 需要调用LLM
        return this.callLLMFromDecoded(input, context, decodedOutput, toolExecutionResult);
   ...
   * 从解码输出执行本地动作
   */
  private executeLocalActionFromDecoded(
    decoded: DecodedOutput,
    context: ConsciousnessContext,
  ): string {
    const localAction = decoded.params?.localAction;
   ...
   * 从解码输出调用 LLM
   */
  private async callLLMFromDecoded(
    input: string,
    context: ConsciousnessContext,
    decoded: DecodedOutput,
    toolExecutionResult?: ToolExecutionResultInfo | null,
    ...
    const systemPrompt = this.buildSystemPrompt(
      context,
      {
        ...
        input: input,
        timestamp: Date.now(),
        finalThoughts: decoded.params?.prompt || '',
        thinkingChain: [],
        detectedBiases: [],
        selfQuestions: [],
        appliedStrategies: [],
