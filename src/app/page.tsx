...
// 导航链接
const NAV_LINKS = [
  { href: '/consciousness', label: '意识系统', icon: Brain },
  { href: '/field-vision', label: '场域视觉', icon: Hexagon },
  { href: '/resonance', label: '共振引擎', icon: Activity },
  { href: '/octahedron-snn', label: '八面体SNN', icon: Triangle },
  { href: '/experiment', label: '对比实验', icon: Layers },
];
...
export default function Home() {
  ...
  const getCapabilityIcon = (type: string) => {
    switch (type) {
      ...
      case 'browser_action':
        return <Monitor className="w-5 h-5 text-orange-400" />;
      default:
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <header className="border-b border-border bg-card/50 backdrop-blur-sm sticky top-0 z-10">
        ...
        {/* 移动端导航 */}
        {showNav && (
          <div className="md:hidden border-t border-border bg-card/50 px-4 py-3">
            <nav className="flex flex-col gap-2">
              {NAV_LINKS.map((link) => (
                <Link
                  ...
                  className="text-sm text-muted-foreground hover:text-foreground flex items-center gap-2 py-2"
                >
                  <link.icon className="w-4 h-4" />
                  {link.label}
                </Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="lg:col-span-1 bg-card/50 border-border">
            <CardHeader>
              ...
              <CardDescription>无需预设工具，LLM动态编排</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {capabilities.map((cap) => (
                  <div key={cap.type} className="p-4 rounded-lg bg-muted/50 border border-border">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <Card className="bg-card/50 border-border">
              <CardHeader>
                ...
                <CardDescription>输入任何任务，Agent会自动选择合适的能力</CardDescription>
              </CardHeader>
              <CardContent>
