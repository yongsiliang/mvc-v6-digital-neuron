...
import { 
  Bot, Send, Loader2, Brain, Heart,
  ChevronDown, ChevronUp, Menu, X,
  Hexagon, Activity, Triangle, Layers,
  Clock, MessageCircle, Lightbulb,
  Cpu, Sparkles, Zap, Settings2, MoreHorizontal,
  Sparkle, Command, Globe, Wand2, Search, Rocket
} from 'lucide-react';
          ...
          if (line.startsWith('data: ')) {
            try {
              ...
              if (data.type === 'content' && data.data?.delta) {
                ...
                thinkingSteps = data.data.chain.slice(0, 3).map((s: { content: string }) => 
                ...
                if (data.data.type === 'action' && data.data.tool) {
                  ...
                  if (tool === 'screenshot' && inTauri) {
                    ...
                    import('@tauri-apps/api/core').then(async ({ invoke }) => {
                      try {
                        const result = await invoke<{ success: boolean; base64: string | null; error: string | null }>('screenshot');
        ...
        <div className="max-w-4xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-2.5 group">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] text-muted-foreground leading-tight hidden sm:block">智能助手</span>
            </div>
          </Link>
          ...
          <div className="flex items-center gap-1">
            ...
            <div className={cn(
              ...
              <Wand2 className={cn("w-3.5 h-3.5", autonomousMode ? "text-amber-400" : "text-muted-foreground")} />
            ...
            <div className={cn(
              ...
              <Settings2 className={cn("w-3.5 h-3.5", devMode ? "text-purple-400" : "text-muted-foreground")} />
          ...
          <div className="max-w-4xl mx-auto px-4 py-2.5">
            <div className="flex flex-wrap items-center gap-3 sm:gap-4 text-xs">
              <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-purple-500/10">
                ...
                <span className="font-medium text-foreground">{Math.floor(existenceStatus.age / 60000)}m</span>
              </div>
              ...
              <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-green-500/10">
                ...
                <span className="font-medium text-foreground">{existenceStatus.conversationCount}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {messages.length === 0 ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center px-4 py-8 sm:py-12">
            ...
            <div className="relative mb-6">
              ...
              <div className="absolute -bottom-1 -left-2 w-5 h-5 rounded-full bg-gradient-to-br from-cyan-400 to-blue-500 shadow-lg" />
            </div>
            ...
            <div className="w-full max-w-md space-y-3">
              <p className="text-xs text-muted-foreground text-center mb-3">
                {autonomousMode ? '试着让我帮你完成这些任务' : '试着问我这些问题'}
              </p>
          ...
          <div className="mb-1.5 flex flex-wrap gap-1">
            {message.emotion.activeEmotions.slice(0, 3).map((e, i) => (
              <Badge 
                key={i} 
                variant="outline" 
                className="text-[10px] px-2 py-0.5 bg-muted/30"
              >
                {e.emotion} <span className="text-muted-foreground ml-0.5">{(e.intensity * 100).toFixed(0)}%</span>
              </Badge>
            ))}
          </div>
        )}
              ...
              {message.learning && (
                <div className="px-3 py-2 rounded-lg bg-green-500/5 border border-green-500/10">
                  ...
                  {message.learning.newConcepts?.length > 0 && (
                    <p className="text-xs text-muted-foreground">新概念: {message.learning.newConcepts.join(', ')}</p>
                  )}
                  {message.learning.newBeliefs?.length > 0 && (
                    <p className="text-xs text-muted-foreground">新信念: {message.learning.newBeliefs.join(', ')}</p>
                  )}
                </div>
              )}
            </div>
